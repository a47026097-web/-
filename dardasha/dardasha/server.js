...
const q = {
  ...
  allUsers: db.prepare(
    `SELECT id, username, display_name, about, last_seen
     FROM users
     WHERE id != ?
     ORDER BY display_name`
  ),

  touchSeen: db.prepare(
...
function auth(req, res, next) {
  ...
  if (!payload) {
    return res.status(401).json({
      error: 'الجلسة منتهية، سجّل الدخول من جديد'
    });
  }
...
app.post('/api/register', (req, res) => {
  ...
  if (!/^[a-z0-9_]{3,20}$/.test(username)) {
    return res.status(400).json({
      error:
        'اسم المستخدم: 3-20 حرف إنكليزي أو رقم أو _'
    });
  }
  ...
  if (displayName.length < 2) {
    return res.status(400).json({
      error:
        'اكتب اسمك الظاهر (حرفين على الأقل)'
    });
  }
  ...
  if (password.length < 6) {
    return res.status(400).json({
      error:
        'كلمة السر لازم 6 أحرف على الأقل'
    });
  }
...
io.on(
  ...
  (socket) => {
    ...
    /*
      السيرفر هنا يعمل Signaling فقط.
      الصوت نفسه ينتقل عبر WebRTC.
    */
    ...
    socket.on(
      ...
      ({ to, sdp }) => {
        ...
        /*
          الإصلاح المهم:
          نرسل SDP مع الدعوة للطرف الثاني.
        */
    ...
    socket.on(
      ...
      () => {
        ...
        if (left <= 0) {
          ...
          /*
            إذا كان المستخدم بمكالمة
            وسكر المتصفح، نخبر الطرف الثاني.
          */
...
server.listen(
  ...
  () => {
    console.log(
      `دردشة تشتغل على http://localhost:${PORT}`
    );
  }
);
